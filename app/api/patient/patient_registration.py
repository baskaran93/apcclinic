...
def register_patient(patient_register: PatientRegister, db: Session = Depends(get_db),
    ...
    # The patient_id is derived from max(id)+1 with no locking, so two
    # concurrent registrations can compute the same next ID; retry a few
    # times on that specific collision rather than failing the request.
    max_attempts = 5
    for attempt in range(max_attempts):
        try:
            ...
            new_patient = PatientDetails(
                ...
                alternative_number = patient_register.alternative_number,
